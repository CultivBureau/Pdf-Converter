...
interface AddIncludesAllOptionModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function AddIncludesAllOptionModal({
  isOpen,
  onClose,
  onSuccess,
  language = "ar",
}: AddIncludesAllOptionModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!optionText.trim()) {
      setError(language === 'ar' ? 'نص الخيار مطلوب' : 'Option text is required');
      return;
    }
    ...
    try {
      ...
      onSuccess(optionText.trim());
      setOptionText("");
      onClose();
    } catch (err) {
      setError(err instanceof Error ? err.message : (language === 'ar' ? 'فشل في إضافة الخيار' : 'Failed to add option'));
    } finally {
      setIsSubmitting(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              {language === 'ar' ? 'نص الخيار' : 'Option Text'} *
            </label>
            <input
              type="text"
              value={optionText}
              ...
              placeholder={language === 'ar' ? 'أدخل نص الخيار (مثال: Includes All)' : 'Enter option text (e.g., Includes All)'}
              autoFocus
            />
            {error && (
